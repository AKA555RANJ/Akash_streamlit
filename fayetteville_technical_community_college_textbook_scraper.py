...
BASE_URL    = "https://bookstore.faytechcc.edu"
LISTING_URL = BASE_URL + "/find-courses"
AUTO_URL    = BASE_URL + "/find-courses/autocomplete"

SCHOOL_ID   = "3055618"
SCHOOL_NAME = "fayetteville_technical_community_college"
...
def fetch_all_sections(sess, term_id):
    ...
    for q in AUTOCOMPLETE_Q:
        try:
            ...
            r = sess.get(AUTO_URL, params={"tid": term_id, "q": q}, timeout=15)
            r.raise_for_status()
            for item in r.json():
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Scrape textbooks from FTCC bookstore.")
