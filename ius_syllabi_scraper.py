#!/usr/bin/env python3
"""
ius_syllabi_scraper.py — Scrape course syllabi from Indiana University Southeast
(IPEDS 3029190) at https://syllabi.iu.edu/

The site uses a JSON REST API (FOSE framework). We search for all Southeast
...
Outputs HTML files + a 17-column CSV to:
  data/indiana_university_southeast__3029190__syllabus/
"""
...
OUTPUT_DIR = os.path.join(
    ...
    "data",
    "indiana_university_southeast__3029190__syllabus",
)
CSV_FILENAME = "indiana_university_southeast__3029190__syllabus.csv"
...
def search_sections(session: requests.Session) -> list[dict]:
    """Search for all Southeast Campus sections in the given term."""
    payload = {
        "other": {"srcdb": SRCDB},
        "criteria": [
            {"field": "alias", "value": "*"},
            {"field": "campus", "value": "Southeast Campus"},
        ],
    }
    resp = session.post(
        API_URL + "search",
...
def main():
    parser = argparse.ArgumentParser(
        description="Scrape IU Southeast syllabi from syllabi.iu.edu"
    )
    parser.add_argument(
        "--no-download",
        action="store_true",
    ...
    for section in tqdm(sections, desc="Processing sections", unit="section"):
        ...
        if existing:
            ...
            rows.append({
                ...
                "syllabus_filepath_local": f"../data/indiana_university_southeast__{SCHOOL_ID}__syllabus/{ex_filename}",
        ...
        rows.append({
            ...
            "syllabus_filepath_local": f"../data/indiana_university_southeast__{SCHOOL_ID}__syllabus/{filename}",
            "syllabus_filesize": str(filesize),
