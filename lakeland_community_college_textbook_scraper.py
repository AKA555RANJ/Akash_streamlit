...
BASE_URL = "https://bookstore.lakelandcc.edu"
COLLEGE_URL = BASE_URL + "/timber/college"
AJAX_URL = BASE_URL + "/timber/college/ajax"

SCHOOL_ID = "3073753"
SCHOOL_NAME = "lakeland_community_college"
...
def fetch_sections(session, course_id, delay=DEFAULT_DELAY):
    ...
    for item in soup.select("div.item.type-college_section a.tcc-item-link"):
        ...
        if len(parts) == 2:
            ...
            section_code = re.sub(r"\s*-\s*$", "", text).strip()
            instructor = ""
        if section_code:
...
def parse_books_html(html, section_id):
    ...
    for req_div in soup.select("div.req-group"):
        ...
        for item_div in req_div.select("div.item.group"):
            ...
            isbn = _extract_isbn(sku_span.get_text(strip=True)) if sku_span else ""
...
def main():
    parser = argparse.ArgumentParser(
        description="Scrape textbook data from Lakeland Community College bookstore (Timber/Drupal)."
    )
