...
SCHOOL_NAME = "mercyhurst_university"
SCHOOL_ID   = "3083429"
BASE_URL    = "https://bookstore.mercyhurst.edu"
...
def parse_term_items(html):
    ...
    items = []
    for div in soup.find_all("div", class_=re.compile(r"type-college_term")):
...
def parse_dept_items(html):
    ...
    for div in soup.find_all("div", class_=re.compile(r"type-college_dept")):
        ...
        if a:
            ...
            if not abbrev:
                abbrev = a.get_text(" ", strip=True).split(" - ")[0].strip()
            if url_path:
...
def discover_terms(sess):
    ...
    if "Just a moment" in resp.text or resp.status_code in (403, 503):
        print("[!] Cloudflare challenge detected. Plain requests blocked.")
        print("    Consider using FlareSolverr or curl_cffi. Exiting.")
        sys.exit(1)
...
def scrape(fresh=False):
    ...
    for term_path, term_label in terms:
        n = scrape_term(sess, term_path, term_label, done_keys, crawled_on)
        total_rows += n
