...
SCHOOL_NAME = "northeast_wisconsin_technical_college"
SCHOOL_ID = "3112231"
BASE_URL = "https://www.nwtcbookstore.com"
...
FLARESOLVERR_SESSION = "nwtc_scraper"

_TERM_SUFFIX_RE = re.compile(
    r'\s*\((?:Order Now|Pre-?Order)[^)]*\)\s*$',
    flags=re.IGNORECASE,
)
...
def create_session():
    ...
    print(f"[DEBUG] HTML length: {len(html)}, token: {form_token[:30] if form_token else 'NONE'}, "
          f"cf_clearance: {'YES' if cookies.get('cf_clearance') else 'NO'}", flush=True)
    with open("/tmp/nwtc_create_session.html", "w") as _f:
        _f.write(html)

    if not form_token:
        raise RuntimeError("Could not extract __RequestVerificationToken from page or cookies")
...
def parse_materials_html(html, term_name):
    ...
    for card in course_cards:
        ...
        raw_section = _input_val(header, "ga4-course-sectionNumber")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if not materials and html:
                        debug_path = f"/tmp/nwtc_debug_batch{batch_idx}.html"
