...
SCHOOL_ID   = "2987566"
STORE_SLUG  = "freddiesbooksandmorestore"
...
FLARESOLVERR_SESSION = "montevallo_bkstr_scraper"

# Spring 2026 is not returned by courseMaterial/info API — hardcoded fallback
FALLBACK_TERMS = [
    {"termId": "100091077", "termName": "Spring 2026", "programId": "5143"},
]
...
def fetch_terms(sess, store_id):
    ...
    for ft in FALLBACK_TERMS:
        if ft["termId"] not in seen_ids:
            terms.append(ft)
            print(f"    [+] Injected fallback term: {ft['termName']} ({ft['termId']})")
